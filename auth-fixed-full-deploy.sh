#!/bin/bash
# AUTH-FIXED-FULL-DEPLOY.SH
# Full application deployment script with authentication fixes
# This builds and deploys the complete application with working authentication

set -e  # Exit on error

echo "🥒 PICKLE+ FULL APPLICATION DEPLOYMENT WITH AUTH FIX 🥒"
echo "====================================================="
echo "Creating a complete deployment with authentication fixed"
...
# Copy client build if exists, or copy public files
if [ -d "client/dist" ]; then
  ...
  echo "Client build not found, copying public files instead..."
...
EOL
  ...
  # Create minimal asset directories
  mkdir -p dist/client/assets
  
  # Create minimal CSS
  cat > dist/client/assets/app.css << 'EOL'
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  root.innerHTML = '<div class="loading">Loading Pickle+ application...</div>';
  ...
  fetch('/api/health')
    ...
    .then(data => {
      root.innerHTML = `
        <div style="max-width: 800px; margin: 40px auto; padding: 20px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
          <h1 style="color: #38b000;">Pickle+ Server Running</h1>
          <div style="background: #f0f8ff; padding: 15px; border-radius: 4px; margin: 20px 0;">
            <p><strong>Status:</strong> ${data.status}</p>
            <p><strong>Database:</strong> ${data.database}</p>
            <p><strong>Auth:</strong> ${data.auth}</p>
          </div>
          <p>Your Pickle+ server is running correctly. This page is a placeholder until the full client application is built.</p>
        </div>
      `;
    })
    .catch(error => {
      root.innerHTML = `
        <div style="max-width: 800px; margin: 40px auto; padding: 20px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
          <h1 style="color: #d32f2f;">Pickle+ Server Error</h1>
          <div style="background: #fff8f8; padding: 15px; border-radius: 4px; margin: 20px 0; border-left: 4px solid #d32f2f;">
            <p>Could not connect to the Pickle+ server API. Please check server logs.</p>
          </div>
        </div>
      `;
    });
...
# Create server.js with authentication fixes
...
echo ""
echo "🥒 PICKLE+ FULL APPLICATION DEPLOYMENT READY 🥒"
echo "=============================================="
echo "To deploy to Replit Cloud Run:"
echo "1. Click the Deploy button in Replit"
echo "2. Set the Build Command to: bash auth-fixed-full-deploy.sh"
...
echo "with robust authentication handling and database connectivity."