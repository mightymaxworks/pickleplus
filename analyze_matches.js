...
async function analyzeMatches() {
  ...
  // Parse matches (skip header rows)
  ...
  const playerQuery = `
    SELECT id, username, "passportCode", gender, "dateOfBirth", "rankingPoints"
    FROM users
    WHERE "passportCode" = ANY($1::text[])
  `;
  
  const { rows: players } = await pool.query(playerQuery, [codesArray]);
  const playerMap = {};
  players.forEach(p => {
    playerMap[p.passportCode] = p;
  });
  
  console.log(`Found ${players.length} players in database`);
  console.log('');
  
  // Check for typos/similar codes
  console.log('🔧 PASSPORT CODE ANALYSIS:');
  const notFoundCodes = codesArray.filter(code => !playerMap[code]);
  if (notFoundCodes.length > 0) {
    console.log(`⚠️  ${notFoundCodes.length} passport codes NOT FOUND in database:`);
    for (const code of notFoundCodes) {
      console.log(`   - ${code} (appears ${codeOccurrences[code]} times)`);
      
      // Find similar codes in database
      const allDBCodes = await pool.query('SELECT "passportCode" FROM users WHERE "passportCode" IS NOT NULL');
      const similarCodes = allDBCodes.rows
        .map(r => r.passportCode)
        .filter(dbCode => {
      ...
      if (similarCodes.length > 0) {
        console.log(`     💡 Possible typo? Similar codes in database:`);
        for (const similar of similarCodes.slice(0, 3)) {
          const playerInfo = await pool.query('SELECT username FROM users WHERE "passportCode" = $1', [similar]);
          console.log(`        → ${similar} (player: ${playerInfo.rows[0]?.username || 'unknown'})`);
        }
      }
    }
    console.log('');
  ...
  // Analyze first 5 matches in detail
  console.log('='.repeat(80));
  ...
  for (const match of matches.slice(0, 5)) {
    ...
    console.log(`${'─'.repeat(80)}`);
    console.log(`Score: Team 1 (${match.team1Score}) vs Team 2 (${match.team2Score})`);
    ...
    for (const code of team1Players) {
      ...
      const genderMultiplier = player.gender === 'female' && player.rankingPoints < 1000 ? 1.15 : 1.0;
      const basePoints = isWin ? SYSTEM_B.WIN : SYSTEM_B.LOSS;
      
      const rankingPoints = basePoints * ageMultiplier * genderMultiplier;
      const rankingPointsRounded = Math.round(rankingPoints * 100) / 100;
      
      console.log(`  ✓ ${code} - ${player.username}`);
      console.log(`    Gender: ${player.gender || 'unknown'}`);
      console.log(`    Age: ${age || 'unknown'} (${ageGroup})`);
      console.log(`    Current Points: ${player.rankingPoints || 0}`);
      console.log(`    Result: ${isWin ? 'WIN' : 'LOSS'}`);
      console.log(`    Calculation: ${basePoints} (base) × ${ageMultiplier} (age) × ${genderMultiplier} (gender) = ${rankingPointsRounded} points`);
      console.log(`    New Total: ${player.rankingPoints || 0} + ${rankingPointsRounded} = ${(player.rankingPoints || 0) + rankingPointsRounded}`);
    }
