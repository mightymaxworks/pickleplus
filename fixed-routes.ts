...
export async function registerRoutes(app: express.Express): Promise<Server> {
  ...
  // PKL-278651-STATS-0002-RD: Direct Implementation
  // CRITICAL: Register direct implementations for performance, rating and ranking endpoints
  // FIRST before any other routes are registered to ensure they take precedence
  console.log("[API][CRITICAL] Registering direct handler implementations for API endpoints");
  
  /**
   * CourtIQ Performance endpoint - DIRECT IMPLEMENTATION
  ...
  app.get('/api/courtiq/performance', (req: Request, res: Response) => {
    ...
    try {
      ...
      console.error('[API][CourtIQ][DIRECT] Error retrieving performance data:', error);
      res.status(500).json({ 
        error: "Server error",
  ...
  /**
   * User Rating Detail endpoint - DIRECT IMPLEMENTATION
  ...
  app.get('/api/user/rating-detail', (req: Request, res: Response) => {
    ...
    try {
      ...
      console.error('[API][UserRating][DIRECT] Error retrieving rating detail:', error);
      res.status(500).json({ 
        error: "Server error",
  ...
  // Now register the standard API route modules
  ...
  // Remove duplicate implementation of this endpoint as we've already moved it to the top of the file
