/**
 * Ranking SDK - Client SDK for the Ranking System
 * 
 * This file implements the client-side SDK for interacting with the ranking system API.
 * It provides functions for retrieving ranking information and performing related operations.
 */
import { 
  RankingTransaction, 
  RankingTierHistory
 ...
 */
export async function getRankingLeaderboard(limit = 10, offset = 0): Promise<any> {
  try {
    const response = await apiRequest("GET", `/api/ranking/leaderboard?limit=${limit}&offset=${offset}`);
    const data = await response.json();
    return data;
  } catch (error) {
 ...
 * @returns Promise with user's leaderboard position
 */
export async function getUserRankingPosition(userId: number): Promise<number> {
  try {
    const response = await apiRequest("GET", `/api/ranking/${userId}/position`);
    const data = await response.json();
...
/**
 * Hook to get ranking leaderboard data
 */
export function useRankingLeaderboard(limit = 10, offset = 0) {
  return {
    queryKey: ['/api/ranking/leaderboard', limit, offset],
    queryFn: async () => {
      const response = await apiRequest("GET", `/api/ranking/leaderboard?limit=${limit}&offset=${offset}`);
      return response.json();
    }
...
/**
 * Hook to get a user's position on the leaderboard
 */
export function useUserRankingPosition(userId: number | undefined) {
  return {
    queryKey: ['/api/ranking/position', userId],
    queryFn: async () => {
      if (!userId) return null;
      const response = await apiRequest("GET", `/api/ranking/${userId}/position`);
      return response.json();
