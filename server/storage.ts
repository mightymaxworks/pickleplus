...
export class DatabaseStorage implements IStorage {
  ...
  async getUsersWithRankingPoints(format?: 'singles' | 'mens-doubles' | 'womens-doubles' | 'mixed-doubles-men' | 'mixed-doubles-women'): Promise<User[]> {
    ...
    if (format === 'singles') {
      ...
      // Legacy: get all users with any ranking points (supports backward compatibility)
      filteredQuery = query.where(
        or(
          ...
          gt(users.doublesRankingPoints, 0), // Legacy support
          gt(users.rankingPoints, 0) // Legacy support
        )
  ...
  async createAssessment(assessmentData: any): Promise<any> {
    try {
      ...
      const result = await this.db.insert(matchAssessments).values({
