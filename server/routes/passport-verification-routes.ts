...
// User passport code and QR code routes
// These need to be registered directly on app since they use a different path pattern
function registerUserPassportRoutes(app: express.Express) {
  // Route to get the current user's passport code (authenticated users only)
  app.get("/api/user/passport/code", isAuthenticated, async (req: Request, res: Response) => {
    try {
      if (!req.user) {
        return res.status(401).json({ error: "User not authenticated" });
      }
  ...
  // Route to get the current user's passport QR code (authenticated users only)
  app.get("/api/user/passport/qr", isAuthenticated, async (req: Request, res: Response) => {
    try {
      if (!req.user) {
        return res.status(401).json({ error: "User not authenticated" });
      }
...
export function registerPassportVerificationRoutes(app: express.Express) {
  ...
  // Register user passport routes
  console.log("[API] Registering User Passport routes (PKL-278651-CONN-0004-PASS-REG-UI-PHASE2)");
  registerUserPassportRoutes(app);
}