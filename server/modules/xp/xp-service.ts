...
import { eq, desc, and, gt, sql, count, sum, max, lte } from 'drizzle-orm';
...
export class XpService {
    ...
    try {
      ...
      for (let i = 0; i < levels.length; i++) {
        if (lifetimeXp >= levels[i].xpRequired) {
          ...
          benefits = levels[i].benefits as Record<string, any>;
  ...
  async calculateLevelForXp(xpAmount: number): Promise<number> {
    try {
      const levels = await db
        .select()
        .from(xpLevelThresholds)
        .where(lte(xpLevelThresholds.xpRequired, xpAmount))
  ...
  async runPicklePulseRecalibration(): Promise<void> {
    try {
      ...
      const totalActivities = activityCounts.reduce((sum: number, act) => sum + Number(act.count), 0);
